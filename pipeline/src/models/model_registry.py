...
MLflow model registry using aliases (@champion/@challenger) for GridCast.
"""
import logging
...
class ModelRegistry:
    """Handle MLflow model registry operations using aliases."""
    ...
    def get_all_versions(self) -> List[ModelVersion]:
        try:
            ...
            logger.warning(f"Could not retrieve grid model versions: {e}")
            return []
    ...
    def transition_to_staging(
        ...
        """Tag the new model version as @challenger for grid evaluation."""
        logger.info("🏛️  Registering load forecasting model as @challenger...")
        ...
        if not version:
            ...
            if all_versions:
                ...
                logger.info(f"   ℹ️  run_id not found — defaulting to latest version v{version}")
            else:
        ...
        try:
            ...
            if tags:
                ...
                logger.info(f"   ✓ Added {len(tags)} evaluation tags")
    ...
    def transition_to_production(self, version: str) -> bool:
        """Promote @challenger to @champion for live grid inference."""
    ...
    def print_registry_status(self):
        logger.info("📊 GridCast Registry Status:")
        logger.info(f"   Model: {self.model_name}")

        all_versions = self.get_all_versions()
        logger.info(f"   Total versions tracked: {len(all_versions)}")
    ...
    def get_deployment_uri(self, alias: str = CHAMPION) -> str:
        """Return the load URI for downstream orchestration."""
