...
class ISONETimeFeatureEngineer(BaseEstimator, TransformerMixin):
    ...
    """
    def __init__(self, country_code: str = config.data.country_code , base_temp: float = config.data.thermo_base_temp):
...
def build_preprocessor() -> Pipeline:
    ...
    return Pipeline([
        ('feature_engineer', ISONETimeFeatureEngineer(
            country_code=config.data.country_code, 
            base_temp=config.data.thermo_base_temp
        )),
        ('scaler', RobustScaler()),
    ])
'''
if __name__ == "__main__":
    ...
    # Simple isolated test block
    logger.info("🔧 Testing pipeline assembly...")
    pipeline = build_preprocessor()
    logger.info("✅ Pipeline successfully built with ISONETimeFeatureEngineer and RobustScaler.")
    '''